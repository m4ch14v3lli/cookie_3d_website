...
export function SmoresCard() {
  ...
  const handleClick = () => {
    if (videoRef.current) {
      videoRef.current.playbackRate = 4.0; // Increased to 4x faster for an immediate splash
      videoRef.current.currentTime = 0;
      videoRef.current.play().catch(() => console.log('Video play failed or file missing'));
      setIsPlaying(true);
    }
  };
    ...
    <>
      ...
      >
        <div className="absolute inset-0 rounded-2xl overflow-hidden border border-white/10 bg-[#0a0a0a]">
          <img src="/smores.avif" alt="Smores Cookie animation" className="w-full h-full object-cover" />
        ...
        >
          <div className="text-[#8c8273] uppercase tracking-widest text-[0.6rem] mb-2">No. 02</div>
          <h3 className="font-serif text-3xl text-[#e2d5c3] mb-4">S'mores</h3>
          <p className="text-[#a1998c] text-sm leading-relaxed font-light backdrop-blur-md bg-black/40 p-4 rounded-lg border border-white/10 shadow-lg md:max-w-md">
            Toasted marshmallows, rich chocolate chunks, and a graham cracker crust, all folded into a buttery brown sugar dough.
          </p>
        </div>
